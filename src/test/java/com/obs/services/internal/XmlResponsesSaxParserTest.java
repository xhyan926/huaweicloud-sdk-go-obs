...
//        TestTools.getExternalEnvironment().initiateMultipartUpload(initRequest);
    ...
    @Test
    public void test_LoggingConfigration_sorting() {
        XmlResponsesSaxParser xmlResponsesSaxParser = new XmlResponsesSaxParser();

        String xml = new StringBuilder("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>"
            + "<BucketLoggingStatus xmlns=\"http://obs.myhwclouds.com/doc/2015-06-30/\">"
            + "<Agency>agency-name</Agency>"
            + "<LoggingEnabled>"
            + "<TargetBucket>bucketName</TargetBucket>"
            + "<TargetPrefix>prefix</TargetPrefix>"
            + "<TargetSorting>DAY</TargetSorting>"
            + "<TargetGrants>"
            + "<Grant>"
            + "<Grantee>"
            + "<ID>id</ID>"
        ...
        XmlResponsesSaxParser.BucketLoggingHandler bucketLoggingHandler = xmlResponsesSaxParser.parse(inputStream,
